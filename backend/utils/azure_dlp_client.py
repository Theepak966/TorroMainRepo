...
# OPTIMIZATION: Cache compiled regex patterns at module level to avoid recompiling on every call
import re
_EMAIL_PATTERNS = [
...
def detect_pii_in_column(column_name: str, sample_data: Optional[List[str]] = None) -> Dict:
    """
    Detect PII in column using Azure DLP (Azure Text Analytics) with pattern-based fallback.
    
    Priority:
    1. Try Azure DLP if configured (more accurate, uses ML)
    2. Fallback to pattern-based detection if Azure DLP unavailable or fails
    ...
    """
    # IMPORTANT: Azure DLP is intentionally disabled (performance + throttling/429 issues).
    # Always use pattern-based detection.
...
def _detect_pii_pattern_based(column_name: str, sample_data: Optional[List[str]] = None) -> Dict:
    ...
    if sample_data:
        ...
        # OPTIMIZATION: Use cached compiled regex patterns instead of recompiling on every call
